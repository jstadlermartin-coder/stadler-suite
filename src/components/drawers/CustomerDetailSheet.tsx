...
interface CustomerDetailSheetProps {
  ...
  stage?: StageValue;
  // Initial tab to open
  initialTab?: 'info' | 'bookings' | 'inquiries' | 'communication' | 'portal';
...
export function CustomerDetailSheet({
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'bookings' | 'inquiries' | 'communication' | 'portal'>(initialTab);
  ...
  const [portalWhatsAppSent, setPortalWhatsAppSent] = useState(false);
  ...
  const sendPortalViaEmail = () => {
    if (portalUrl && customer.email) {
      const subject = encodeURIComponent('Ihr persönliches Gästeportal');
      const body = encodeURIComponent(`Hallo ${customer.name},\n\nHier ist Ihr persönlicher Zugang zum Gästeportal:\n${portalUrl}\n\nMit freundlichen Grüßen`);
      window.open(`mailto:${customer.email}?subject=${subject}&body=${body}`);
      setPortalEmailSent(true);
      setTimeout(() => setPortalEmailSent(false), 2000);
    }
  };

  const sendPortalViaWhatsApp = () => {
    if (portalUrl && customer.phone) {
      const message = encodeURIComponent(`Hallo ${customer.name}, hier ist Ihr persönlicher Zugang zum Gästeportal: ${portalUrl}`);
      const phone = customer.phone.replace(/\D/g, '');
      window.open(`https://wa.me/${phone}?text=${message}`);
      setPortalWhatsAppSent(true);
      setTimeout(() => setPortalWhatsAppSent(false), 2000);
    }
  };
        ...
        <div className="flex border-b border-slate-100">
          ...
          {portalToken && (
            <button
              ...
              className={`flex-1 py-3 text-sm font-medium transition-colors ${activeTab === 'portal' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Portal
            </button>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-50 rounded-xl p-5">
                <div className="flex items-center gap-3 text-slate-500 mb-3">
                  ...
                  <span className="text-sm font-medium">Gästeportal-Link</span>
                </div>
                <div className="bg-white border border-slate-200 rounded-lg p-3 mb-4">
                  <p className="text-sm text-slate-700 break-all font-mono">{portalUrl}</p>
                </div>
                <button
                  onClick={copyPortalLink}
                  className="w-full flex items-center justify-center gap-2 py-3 border border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 transition-colors font-medium"
                >
                  {portalLinkCopied ? (
                    <>
              ...
              <div className="bg-blue-50 rounded-xl p-5">
                <div className="flex items-center gap-3 text-blue-700 mb-3">
                  ...
                  <span className="text-sm font-medium">Per E-Mail senden</span>
                </div>
                {customer.email ? (
                  <>
                    <p className="text-sm text-blue-600 mb-3">An: {customer.email}</p>
                    <button
                      onClick={sendPortalViaEmail}
                    ...
                    >
                      {portalEmailSent ? (
                        ...
                        <>
                          ...
                          Portal per E-Mail senden
                        </>
                      )}
                    </button>
                  </>
                ) : (
                  <p className="text-sm text-blue-500">Keine E-Mail-Adresse hinterlegt</p>
                )}
              </div>
              ...
              <div className="bg-green-50 rounded-xl p-5">
                ...
                {customer.phone ? (
                  <>
                    <p className="text-sm text-green-600 mb-3">An: {customer.phone}</p>
                    <button
                      onClick={sendPortalViaWhatsApp}
                    ...
                    >
                      {portalWhatsAppSent ? (
                        ...
                        <>
                          <Send className="h-5 w-5" />
                          Portal per WhatsApp senden
                        </>
                      )}
                    </button>
                  </>
                ) : (
                  <p className="text-sm text-green-500">Keine Telefonnummer hinterlegt</p>
                )}
              </div>
            </div>
          )}
